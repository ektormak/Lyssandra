...
def sparse_group_omp(X,D,Gram,groups=None,n_groups=None,n_nonzero_coefs=None):
	...
	for i in xrange(n_samples):
		Z[:,i] = _sparse_group_omp(X[:,i],D,Gram,Alpha[:,i],groups=groups,n_groups=n_groups,n_nonzero_coefs=n_nonzero_coefs)
	return Z
...
"""
def _batch_omp_sp(x,D,Gram,a0,n_nonzero_coefs):

	#the sparsity constrained bomp
	n_atoms = D.shape[1]
	#the indexes of the atoms this datapoint uses
	...
	for i in xrange(n_nonzero_coefs):
		#find the atom that correlates the
		#most with the residual
		k = np.argmax(np.abs(a))
		...
		if i > 0:
			if i == 1:
				...
				#after second iteration
				#solve for w
				w = solve_triangular(L[:i,:i],g,lower=True,check_finite=False)
				v = np.dot(w,w)
				vs = 1-v
				if vs < _eps:
					break
			...
			I = np.append(I,k)
			try:
				Ltc = solve_triangular(L[:i+1,:i+1],a0[I],lower=True)
				z = solve_triangular(L[:i+1,:i+1],Ltc,trans=1,lower=True)
			except LinAlgError:
				print "!!!Gram matrix is singular due to linear dependencies in the dictionary!!!"
				I = I[:-1]
				break
			a = a0 - np.dot(G[:,I],z)
...
def _batch_omp_error(x,D,Gram,a0,tol):

	#the error constrained bomp
	n_atoms = D.shape[1]
	#the indexes of the atoms this datapoint uses
	I = np.array([]).astype(int)
	G = Gram
	...
	while epsilon > tol:
		...
		if i > 0:
			if i == 1:
				#second iteration
				w = solve_triangular([[1]],g,lower=True,check_finite=False)
				v = w*w
				if 1-v <= np.finfo(float).eps:
					break
				L[:2,:2] = [[1,0],
							[w,np.sqrt(1-v)]]
				...
				L[i,:i] = w
				L[i,i] = np.sqrt(1-v)
			#after second iteration solve for z
			I = np.append(I,k)
			try:
				Ltc = solve_triangular(L[:i+1,:i+1],a0[I],lower=True)
				z = solve_triangular(L[:i+1,:i+1],Ltc,trans=1,lower=True)
		...
		beta = np.dot(G[:,I],z)
		a = a0 - beta
		delta = np.dot(z,beta[I])
		#calculate the error
		epsilon = epsilon - delta + delta_prev
		delta_prev = delta
...
def _grad_desc_step(x,z,D,learning_rate):
	#performs on step of gradient descent(w.r.t to sparse codes while fixing D)
...
"""
def soft_thresh_group(z_group,_lambda):
	nrm2 = get_blas_funcs(['nrm2'],[z_group])[0]
	if nrm2(z_group) <= _lambda:
		return np.zeros((z_group.size))
	else:
		return ( (nrm2(z_group)-_lambda) / nrm2(z_group) ) * z_group
"""
...
class sparse_encoder(object):
	...
	def __call__(self,X,D):
		...
		if self.algorithm == 'omp':
			...
			data = Alpha
			args = []
			func = partial(thresholding,n_nonzero_coefs=self.params.get('n_nonzero_coefs'),nonzero_percentage=self.params.get('nonzero_percentage'))
			...
			Gram = fast_dot(D.T,D)
			data = X
			func = partial(somp,data_groups=self.params.get('data_groups'),n_nonzero_coefs=self.params.get('n_nonzero_coefs'))
			args = [D,Gram]
