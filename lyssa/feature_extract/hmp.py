import numpy as np
from lyssa.utils.img import grid_patches_loc,grid_patches,extract_patches,compute_n_patches
from lyssa.dict_learning import ksvd_coder,online_dictionary_coder
from lyssa.sparse_coding import sparse_encoder
from lyssa.utils.dataset import img_dataset,save,load,online_writer,online_reader,get_mmap,load_sparse_matrix,save_sparse_matrix
from lyssa.utils import workspace_manager,run_parallel,gen_even_batches,get_workspace
	...
	for i in range(n_imgs):
		...
		if conv:
			...
			if pooling_size is not None:
				...
				save_sparse_matrix(os.path.join(output_img_path,"img"+str(idxs[i])+".npy"),sparse.csc_matrix(pooled_img.reshape((pool_h*pool_w,n_features))))
				...
				save_sparse_matrix(os.path.join(output_img_path,"img"+str(idxs[i])+".npy"),sparse.csc_matrix(pooled_img.reshape((ph*pw,n_features))))
				#save the dimensions
...
class hmp_network():
	...
	def __init__(self,dict_learners=None,n_atoms=None,sparse_coders=None,feature_encoders=None,filter_sizes=None,
						...
						spm_levels=(1,2,4),rebuild_spp=False,spm_normalizer=None,spp_pooler=None,n_training_patches=None,imgs=None,
	...
	def show_details(self):
		...
		for l in range(self.n_layers):
			...
			avg_height_after_conv,avg_width_after_conv = compute_n_patches(avg_im_height,avg_im_width,self.filter_sizes[l],self.step_sizes[l],padding=False)
			...
			if self.pooling_sizes[l] is not None:
				avg_height_after_pooling,avg_width_after_pooling = compute_n_patches(avg_height_after_conv,
																					avg_width_after_conv,self.pooling_sizes[l],
																					self.pooling_steps[l],padding=False)
			...
			size_in_gb = compute_training_set_mem(n_training_patches=self.n_training_patches[l],patch_size=self.filter_sizes[l],patch_depth=patch_depth)
	...
	def build_feature_maps(self,layer):
		...
		if (not self.workspace.contains(os.path.join("layer"+str(layer),"dict.npy"))) and self.pretrained_dicts[layer] is None:
		...
		if layer > 0:
			...
			self.imgs[layer] = online_reader(path=os.path.join(self.workspace.base_path,input_img_path),sparse="3D",prefix="img",suffix="npz")
		feature_maps_path = "layer"+str(layer)+"/feature_maps"
		os.path.join("layer"+str(layer),"feature_map_dims.npy")

		if not os.path.exists(os.path.join(self.workspace.base_path ,os.path.join("layer"+str(layer),"feature_map_dims.npy"))):
			...
			for i in range(n_batches):
				...
				X,patch_sizes = extract_patches(self.imgs[layer][batch_idxs[i]],
									...
									mmap=self.mmap,n_jobs=self.n_jobs,scale=False,verbose=False,mem="high")
	...
	def build_layer(self,layer=0):
		...
		if spp or conv:
			...
			func = partial(pool_proc,pooling_size=pooling_size,spp_pooler = self.spp_pooler,
						pooling_step=pooling_step,spp=spp,conv=conv,normalizer=self.spm_normalizer,levels=self.spm_levels)
			n_batches = 100
			msg = "building next layer"
			data = self.feature_maps[layer]
			if spp:

				Z_final = run_parallel(func=func,data=data,args=[output_img_path],batched_args=[feature_map_dims.T,idxs],
